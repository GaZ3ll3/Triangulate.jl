	...
	REAL* pointlist)
{
	...
	jl_gc_enable(0);
#ifdef SINGLE
	jl_datatype_t *dtype = jl_get_global(ModuleType, jl_symbol("triangulateio32"));
#else
	jl_datatype_t *dtype = jl_get_global(ModuleType, jl_symbol("triangulateio64"));
#endif

	jl_value_t* int_array_type = jl_apply_array_type(jl_INT, 1);
